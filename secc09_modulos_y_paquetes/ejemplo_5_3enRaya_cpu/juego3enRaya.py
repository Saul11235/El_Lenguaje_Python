...
class juego3enRaya:
    ...
    def inputCPU(self):
        'Metodo que detiene el programa y recibe una instruccion de CPU'
        if not self.tablero.hayJugadas: return False
        try:
            jugadas_posibles = self.tablero.getJugadasPosibles()
            jugada           = choice(jugadas_posibles)
            return self.tablero.jugar(jugada)
        except:
            return self.inputCPU() #repetir
    ...
    def lanzar1jugador(self):
        self.printMarcador()
        self.printTablero()    
        repetir = True
        while repetir:
            if self.tablero.turnoJug2:
                #--------------------------------
                # caso jugador humano
                repetir = not self.inputJugada()
                #--------------------------------
